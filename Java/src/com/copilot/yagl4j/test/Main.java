...
public class Main implements GamePadListener {
    ...
    public void onGamePadAxisMotion(long gamePadID, byte axis, short value) {
        System.out.print(String.format("%c[%d;0f Controller %d moved axis %s: %d    ", 0x1B, axis + 1, gamePadID, GamePadAxis.fromValue(axis), value));
        System.out.flush();
    }

    @Override
    public void onGamePadButtonDown(long gamePadID, byte button) {
        System.out.println(String.format("%c[%d;0f Controller %d %s: [x]", 0x1B, 7 + button, gamePadID, GamePadButton.fromValue(button)));
    }

    @Override
    public void onGamePadButtonUp(long gamePadID, byte button) {
        System.out.println(String.format("%c[%d;0f Controller %d %s: [ ]", 0x1B, 7 + button, gamePadID, GamePadButton.fromValue(button)));
    }
